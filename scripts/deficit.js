/* =========================================================
   ...
   Debt + average interest rate are pulled LIVE from the
   U.S. Treasury Fiscal Data API (with a hardcoded fallback).
...
(() => {
  ...
  function revealIn(root, instant) {
    const nodes = [...(root || document).querySelectorAll("[data-reveal]:not(.in)")];
    if (instant || prefersReduced || !("IntersectionObserver" in window)) { nodes.forEach((n) => n.classList.add("in")); return; }
  ...
  /* ---- build everything that depends on the (live or fallback) debt + rate ---- */
  function build(fiscal, instant) {
    const DEBT0 = fiscal.debt, RATE = fiscal.rate;
    const ratePct = (RATE * 100).toFixed(2);
    const SERVICE = SURPLUS + DEBT0 * RATE;        // constant annual amount toward interest + principal
    const rows = [];
    let debt = DEBT0, totalInterest = 0, yy = 0;
    while (debt > 0.5 && yy < 400) {
      const interest = debt * RATE;
      const principal = Math.min(debt, SERVICE - interest);
      rows.push({ year: yy + 1, start: debt, interest, principal, end: debt - principal });
      totalInterest += interest;
      debt -= principal;
      yy++;
    }
    const PAYOFF = rows.length;
    const TARGET = YEAR + PAYOFF;

    const srcLine = fiscal.live
      ? `<p class="fiscal-src live" data-reveal>📡 <strong>Live data.</strong> The <strong>${money(DEBT0)}</strong> debt and <strong>${ratePct}%</strong> average interest rate are pulled straight from the <strong>U.S. Treasury</strong> — "Debt to the Penny" and the average rate on the public debt, as of ${fmtDate(fiscal.debtDate)}. <a href="https://fiscaldata.treasury.gov/datasets/debt-to-the-penny/" target="_blank" rel="noopener">Source: fiscaldata.treasury.gov ↗</a></p>`
      : `<p class="fiscal-src" data-reveal>Debt (<strong>${money(DEBT0)}</strong>) and rate (<strong>${ratePct}%</strong>) shown here are illustrative fallbacks — we couldn't reach the live U.S. Treasury feed just now.</p>`;

    /* ---- top: headline + steps + year-1 snapshot ---- */
    const elTop = document.getElementById("deficitTop");
    if (elTop) {
      ...
      elTop.innerHTML = `
        ...
        <div class="debt-clear" data-reveal style="max-width:880px">
          ...
          <div class="dc-body">
            ...
            <p class="dc-note">Interest-aware at the live ${ratePct}% average rate. We'd pay roughly <strong>${money(totalInterest)}</strong> in interest along the way — which is exactly why paying the principal down matters. Faster growth or a bigger surplus pays it off sooner.</p>
          </div>
        </div>
    ...
    const elChart = document.getElementById("deficitChart");
    if (elChart) {
      const W = 760, H = 320, padL = 56, padR = 56, padT = 18, padB = 38;
      const plotW = W - padL - padR, plotH = H - padT - padB;
      const xMax = PAYOFF, yMax = DEBT0, surMax = Math.max(1500, Math.ceil(SERVICE / 500) * 500);
      ...
      let grid = "";
      for (let g = 0; g <= 4; g++) {
        const dv = (yMax / 4) * g, yyv = sy(dv);
        grid += `<line x1="${padL}" y1="${yyv.toFixed(1)}" x2="${W - padR}" y2="${yyv.toFixed(1)}" stroke="#e2e8f2"/>`;
        grid += `<text x="${padL - 8}" y="${(yyv + 4).toFixed(1)}" text-anchor="end" font-size="10" fill="#c8102e" font-family="Oswald">$${(dv / 1000).toFixed(0)}T</text>`;
      }
      let rlab = "";
      for (let s = 0; s <= surMax; s += 500) {
        const yyv = syR(s);
        rlab += `<text x="${W - padR + 8}" y="${(yyv + 4).toFixed(1)}" text-anchor="start" font-size="10" fill="#1f7a4d" font-family="Oswald">${s === 0 ? "$0" : "$" + (s / 1000).toFixed(1) + "T"}</text>`;
      }
      let xlab = "";
      ...
      elChart.innerHTML = `
        ...
        <p class="amort-note" data-reveal>Watch the two middle columns trade places: interest (what we owe the past) falls every year, while the surplus going to principal (what frees the future) climbs — from <strong>${money(SURPLUS)}</strong> in year one to far more near the end. Interest-aware at the live ${ratePct}% rate; faster growth or a bigger surplus would pay it off sooner.</p>`;
    }

    revealIn(elTop, instant);
    revealIn(elChart, instant);
  }

  /* ---- presidents: deficit/surplus track record (independent of live data, rendered once) ---- */
  ...
  /* ---- first paint with the fallback, then upgrade to live Treasury data ---- */
  build({ debt: P.fiscal.debtToday, rate: P.fiscal.interestRate, live: false }, false);

  (async () => {
    const API = "https://api.fiscaldata.treasury.gov/services/api/fiscal_service/v2/accounting/od/";
    const debtURL = API + "debt_to_penny?fields=record_date,tot_pub_debt_out_amt&sort=-record_date&page%5Bsize%5D=1";
    const rateURL = API + "avg_interest_rates?fields=record_date,avg_interest_rate_amt&filter=" +
      encodeURIComponent("security_desc:eq:Total Interest-bearing Debt") + "&sort=-record_date&page%5Bsize%5D=1";
    try {
      const ctrl = new AbortController();
      const to = setTimeout(() => ctrl.abort(), 7000);
      const [dRes, rRes] = await Promise.all([
        fetch(debtURL, { signal: ctrl.signal }),
        fetch(rateURL, { signal: ctrl.signal }),
      ]);
      clearTimeout(to);
      const dJson = await dRes.json();
      const debt = parseFloat(dJson.data[0].tot_pub_debt_out_amt) / 1e9; // -> $B
      const debtDate = dJson.data[0].record_date;
      let rate = P.fiscal.interestRate;
      try { const rJson = await rRes.json(); if (rJson.data && rJson.data[0]) rate = parseFloat(rJson.data[0].avg_interest_rate_amt) / 100; } catch (_) {}
      if (isFinite(debt) && debt > 0) build({ debt, rate, live: true, debtDate }, true);
    } catch (_) {
      /* keep the fallback render */
    }
  })();
})();
